...
Los muros se crean desde los mismos ejes de sketch. Si la API `Arch` esta disponible, intenta crear `Arch Wall`; si falla, crea solidos `Part::Feature` con las mismas propiedades. Los buques de puerta y ventana se crean como solidos de apertura, no como familias parametricas de puerta/ventana.
