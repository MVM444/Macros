"""Smoke test GUI no persistente de FreeCAD-HVAC en FreeCAD 1.1.3.

Esta prueba vive fuera del repositorio oficial FreeCAD-HVAC. Crea un documento
temporal, activa HVAC, genera una red y una ruta minima y cierra el documento
sin guardarlo. El proceso GUI usado para la prueba se cierra al finalizar.
"""
...
RESULT_PATH = os.path.join(
    ...
    "resultado_verificacion_2026-09-02.json",
)
DOCUMENT_NAME = "Codex_HVAC_Smoke_20260902"
...
def _wait_for_deferred_gui_work(milliseconds=1500):
...
def _object_summary(obj):
    ...
    for property_name in ("DuctType", "JunctionType", "NetworkType"):
        if hasattr(obj, property_name):
            value = getattr(obj, property_name)
            summary[property_name] = str(value)
    return summary
...
def verify():
    result = {
        ...
        "hvac_package_path": None,
        "hvac_init_gui_path": None,
    ...
    try:
        ...
        _wait_for_deferred_gui_work(500)
        ...
        import freecad.HVAC as hvac_package
        import freecad.HVAC.init_gui as hvac_init_gui
        import networkx

        result["hvac_package_path"] = os.path.abspath(hvac_package.__file__)
        result["hvac_init_gui_path"] = os.path.abspath(hvac_init_gui.__file__)
        result["networkx_version"] = getattr(networkx, "__version__", None)
        result["networkx_path"] = os.path.abspath(networkx.__file__)
        ...
        if not result["hvac_toolbars"] or not result["hvac_menus"]:
            raise RuntimeError("No se detectaron la barra y el menu HVAC")
        ...
        if doc is not None and doc.Name in App.listDocuments():
            result["document_saved"] = not doc.isTouched() and bool(doc.FileName)
